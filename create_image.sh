...
if [ "$auto" -eq 0 ]; then
    ...
    read size
    echo -ne "Filesystem: exfat and fat32 are supported? "
    read part_type
else
...
if [ "$part_type" != "exfat" ] && [ "$part_type" != "fat32" ]; then
    echo "$part_type is not supported, choose exfat or fat32" 1>&2
    exit 1
fi
...
parted "$dev" mkpart primary 1M 100%

if [ "$part_type" = "exfat" ]; then
    mkfs.exfat -n RPiHDD "${dev}p1"
